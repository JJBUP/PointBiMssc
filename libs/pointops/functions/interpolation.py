...
def interpolation_spp(xyz, new_xyz, feat, offset, new_offset, nsample=3):
    """
    input: coords: (m, 3), new_xyz: (n, 3), color: (m, c), offset: (b), new_offset: (b)
    output: (n, c)
    """
    assert xyz.is_contiguous() and new_xyz.is_contiguous() and feat.is_contiguous()
    idx, dist = knn_query(nsample*3, xyz, offset, new_xyz, new_offset)  # (n, k), (n, k)
    new_feat = torch.cuda.FloatTensor(new_xyz.shape[0], feat.shape[1],3).zero_()
    for i in range(3):
        dist_recip = 1.0 / (dist[:,:nsample*(i+1)] + 1e-8)  # (n, k)
        norm = torch.sum(dist_recip, dim=1, keepdim=True)
        weight = dist_recip / norm  # (n, k)
        for j in range(nsample*(i+1)):
            new_feat[:,:,i] += feat[idx[:, j].long(), :] * weight[:, j].unsqueeze(-1)
    return new_feat # [n c 3]
