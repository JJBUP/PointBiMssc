...
class PointTransformerLayer(nn.Module):
    def __init__(self, in_planes, out_planes, share_planes=8, nsample=16):
        ...
        self.linear_p = nn.Sequential(nn.Linear(6, 6),
                                      LayerNorm1d(6),
                                      nn.ReLU(inplace=True),
                                      nn.Linear(6, out_planes))
    ...
    def forward(self, pxo) -> torch.Tensor:
        ...
        p_r, k = k[:, :, 0:3], k[:, :, 3:]
        p_r = self.linear_p(torch.cat([p_r, self.xyz2sphere(p_r)], dim=-1))  # N G D
        w = k - q.unsqueeze(1) + p_r  # N G D
        w = self.softmax(self.linear_f_w(w))  # (n, nsample, c)
        feat = torch.einsum("n t s i, n t i -> n s i",
                            einops.rearrange(v + p_r, "n ns (s i) -> n ns s i", s=self.share_planes), w)
        ...
        post = torch.einsum("n t s i, n t i -> n s i",
                            einops.rearrange(v + x, "n ns (s i) -> n ns s i", s=self.share_planes), w)
        # del v, w, x
...
class TransitionDown(nn.Module):
    def __init__(self, in_planes, out_planes, stride=1, nsample=16):
        ...
        if stride != 1:
            ...
            self.pool1 = nn.MaxPool1d(self.nsample)
            self.pool2 = nn.MaxPool1d(3 * self.nsample)
            self.pool3 = nn.MaxPool1d(8 * self.nsample)
    ...
    def forward(self, pxo):
        ...
        if self.stride != 1:
            ...
            n_p = p[idx.long(), :]  # (m, 3)
            # scannet
            x, _ = pointops.knn_query_and_group(x, p, offset=o, new_xyz=n_p, new_offset=n_o, nsample=8 * self.nsample,  # 原来是9
                                                with_xyz=True)
            x = torch.concat([self.pool1(
                self.relu(self.bn1(self.linear1(x[:, :1 * self.nsample]).transpose(1, 2).contiguous()))).squeeze(-1),
                              self.pool2(self.relu(self.bn2(
                                  self.linear2(x[:, :3 * self.nsample]).transpose(1, 2).contiguous()))).squeeze(-1),
...
@MODELS.register_module("PointBiMssc_scv2")
class PointBiMssc(nn.Module):
    def __init__(self, in_channels=None, num_classes=None, block=Bottleneck, blocks=(1, 2, 3, 5, 2)):
        super().__init__()
