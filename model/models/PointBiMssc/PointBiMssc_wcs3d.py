...
class PointTransformerLayer(nn.Module):
    def __init__(self, in_planes, out_planes, share_planes=8, nsample=16):
        ...
        self.linear_p = nn.Sequential(nn.Linear(6, 12),
                                      LayerNorm1d(12),
                                      nn.ReLU(inplace=True),
                                      nn.Linear(12, out_planes)
                                      )
...
class TransitionDown(nn.Module):
    def __init__(self, in_planes, out_planes, stride=1, nsample=16):
        ...
        if stride != 1:
            ...
            self.pool2 = nn.MaxPool1d(2 * self.nsample)
            self.pool3 = nn.MaxPool1d(3 * self.nsample)
    ...
    def forward(self, pxo):
        ...
        if self.stride != 1:
            ...
            x, _ = pointops.knn_query_and_group(x, p, offset=o, new_xyz=n_p, new_offset=n_o, nsample=3 * self.nsample,
                                                with_xyz=True)
            ...
            x = torch.concat([self.pool1(
                self.relu(self.bn1(self.linear1(x[:, :1 * self.nsample]).transpose(1, 2).contiguous()))).squeeze(-1),
                              self.pool2(self.relu(self.bn2(
                                  self.linear2(x[:, :2 * self.nsample]).transpose(1, 2).contiguous()))).squeeze(-1),
...
@MODELS.register_module("PointBiMssc_wcs3d")
