...
class SegmentationTest(object):
    ...
    def __call__(self, cfg, test_loader, model):
        ...
        colors = np.array([cfg.data.color_dict[name] for name in cfg.data.names])
        for idx in range(len(test_dataset)):
            end = time.time()
            data_name = test_dataset.get_data_name(idx)
            pred_save_path = os.path.join(save_path, '{}_pred.txt'.format(data_name))
            label_save_path = os.path.join(save_path, '{}_label.txt'.format(data_name))
            ...
            # else:
            data_dict_list, label, ori_data = test_dataset[idx]
            ...
            for i in range(batch_num):
                ...
                for be in input_dict["offset"]:
                    ...
                    bs = be
                logger.info('Test: {}/{}-{data_name}, Batch: {batch_idx}/{batch_num} [{batch_size}]'.format(
                    idx + 1, len(test_dataset), data_name=data_name, batch_idx=i + 1, batch_num=batch_num,
                    batch_size=e_i - s_i))
            pred = pred.argmax(1).data.cpu().numpy()
            if test_dataset.save_complete_pred:
                # others_color = (224, 224, 224)
                # pred_color = test_dataset.color_orderlist[pred]
                # others_idx = np.where(label == test_dataset.ignore_index, True, False)
                # pred_color[others_idx] = 224
                # np.savetxt(pred_save_path,
                #            np.concatenate((ori_data, pred_color, pred.reshape(-1, 1)), axis=1),
                #            fmt='%.4f,%.4f,%.4f,%d,%d,%d,%d')
                pc = np.concatenate((ori_data, colors[pred], pred.reshape(-1, 1)), axis=1)
                lc = np.concatenate((ori_data, colors[label], label.reshape(-1, 1)), axis=1)
                if cfg.dataset_type == "ScanNetDataset":
                    others_rgb = (224, 224, 224)
                    others_idx = np.where(label == cfg.data["ignore_index"], True, False)
                    pc[others_idx][0], pc[others_idx][1], pc[others_idx][2] = others_rgb[0], others_rgb[1], others_rgb[2],
                np.savetxt(pred_save_path, pc, fmt='%.4f,%.4f,%.4f,%d,%d,%d,%d')
                np.savetxt(label_save_path, lc, fmt='%.4f,%.4f,%.4f,%d,%d,%d,%d')
            else:
