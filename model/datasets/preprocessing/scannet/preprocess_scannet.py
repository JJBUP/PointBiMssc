...
def handle_process(scene_path, output_path, labels_pd, train_scenes, val_scenes, parse_normals=True):
    ...
    mesh_path = os.path.join(scene_path, f'{scene_id}{CLOUD_FILE_PFIX}.ply')  # '_vh_clean_2'
    segments_file = os.path.join(scene_path,
                                 f'{scene_id}{CLOUD_FILE_PFIX}{SEGMENTS_FILE_PFIX}')  # '.0.010000.segs.json'
    aggregations_file = os.path.join(scene_path, f'{scene_id}{AGGREGATIONS_FILE_PFIX}')  # '.aggregation.json'
    info_file = os.path.join(scene_path, f'{scene_id}.txt')

    if scene_id in train_scenes:
        output_file = os.path.join(output_path, 'train', f'{scene_id}.txt')
        split_name = 'train'
    elif scene_id in val_scenes:
        output_file = os.path.join(output_path, 'val', f'{scene_id}.txt')
        split_name = 'val'
    else:
        output_file = os.path.join(output_path, 'test', f'{scene_id}.txt')
        split_name = 'test'

    print(f'Processing: {scene_id} in {split_name}')

    vertices, faces = read_plymesh(mesh_path)  # 读取ply文件
    coords = vertices[:, :3]  # 坐标
    colors = vertices[:, 3:6]  # rgb
    ...
    if split_name != "test":
        label = np.stack([save_dict["semantic_gt20"], save_dict["semantic_gt200"], save_dict["instance_gt"]], axis=1)
        data = np.concatenate([save_dict["coord"], save_dict["color"], save_dict["normal"], label], axis=1)
        np.savetxt(output_file, data, "%.8f %.8f %.8f %d %d %d %.8f %.8f %.8f %d %d %d")
    else:
        # Save processed data
        data = np.concatenate([save_dict["coord"], save_dict["color"], save_dict["normal"]], axis=1)
        np.savetxt(output_file, data, "%.8f %.8f %.8f %d %d %d %.8f %.8f %.8f")
    # torch.save(save_dict, output_file)


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--dataset_root', default="/home/data/scannet/scannet_v2_semseg/",
                        help='Path to the ScanNet dataset containing scene folders')
    parser.add_argument('--output_root', default="/home/data/mx/project/DATASET/scannetv2_preprocess_text/",
                        help='Output path where train/val folders will be located')
    parser.add_argument('--parse_normals', default=True, type=bool, help='Whether parse point normals')
    config = parser.parse_args()

    # Load label map
    # labels_pd = pd.read_csv('model/datasets/preprocessing/scannet/meta_data/scannetv2-labels.combined.tsv',
    labels_pd = pd.read_csv('meta_data/scannetv2-labels.combined.tsv',
                            sep='\t', header=0)

    # Load train/val splits
    # with open('model/datasets/preprocessing/scannet/meta_data/scannetv2_train.txt') as train_file:
    with open('meta_data/scannetv2_train.txt') as train_file:
        train_scenes = train_file.read().splitlines()
    # with open('model/datasets/preprocessing/scannet/meta_data/scannetv2_val.txt') as val_file:
