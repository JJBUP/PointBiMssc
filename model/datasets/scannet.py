...
class ScanNetDataset(Dataset):
    ...
    color_order_list = np.array([SCANNET_COLOR_MAP_20[i] for i in VALID_CLASS_IDS_20])
    ...
    def __init__(self,
        ...
        self.save_complete_pred = save_complete_pred  # 测试中是否保存带标签的完整数据
    ...
    def get_data(self, idx):
        ...
        if not self.cache:
            ...
            cache_name = "model" + data_name.replace(os.path.sep, "-")
    ...
    def prepare_test_data(self, idx):
        ...
        for data in data_dict_list:
            data_part_list = self.test_voxelize(data)  # 体素化
            for data_part in data_part_list:  # 最大点云限制
                if self.test_crop:
