...
class Collect(object):
    def __init__(self,
        ...
        self.keys = keys  # data_dict中保留的数据，如coord，label
        self.offset_keys = offset_keys_dict  # 用于根据数据长度生成offset的值，这里的值为数据长度
        self.kwargs = kwargs  # feat:初始合成feat数据所融合的名字，如coord,color,normal

    def __call__(self, data_dict):
        data = dict()
        if isinstance(self.keys, str):
            self.keys = [self.keys]  # 转换为列表
        for key in self.keys:
            data[key] = data_dict[key]  # 存入新字典
        for key, value in self.offset_keys.items():  # offset
            # 下面的处理过程可知，value只能是str而不是列表
            data[key] = torch.tensor([data_dict[value].shape[0]])
        # 合成feat
        for name, keys in self.kwargs.items():
            name = name.replace("_keys", "")  # feat_keys ->feat
...
class ToTensor(object):
    def __call__(self, data):
        ...
        elif isinstance(data, np.ndarray) and np.issubdtype(data.dtype, np.int_):
...
class Voxelize(object):
    def __init__(self,
        ...
        self.mode = mode
        self.keys = keys
        self.return_discrete_coord = return_discrete_coord  # 网格坐标作为特征
        self.return_min_coord = return_min_coord  # 数据最小值作为特征

    def __call__(self, data_dict):
        assert "coord" in data_dict.keys()
        # 体素坐标,无最场景最大体素在SphereCrop
        discrete_coord = np.floor(data_dict["coord"] / np.array(self.voxel_size)).astype(np.int_)
...
class SphereCrop(object):
    def __init__(self, point_max=80000, sample_rate=None, mode="random"):
        ...
        assert mode in ["random", "center", "all"]  # train val test
        self.mode = mode

    def __call__(self, data_dict):
        # pointmax可指定也可以根据场景中点云数量按比例指定
        ...
        if self.mode == "all":
            ...
            if data_dict["coord"].shape[0] > point_max:
                # 随机选点，计算距离，筛选距离最近的pointmax个点，直到idx_uni点都被标记
